...
export default async (viteConfig,  callback) => {
  ...
  try {
    ...
    await build({
      ...
      build: {
        emptyOutDir: true,
        outDir: theirStudioConfig.outputPath ? resolve(viteConfig.cwd, theirStudioConfig.outputPath) : resolve(viteConfig.cwd, '/dist'),
        rollupOptions: {
          input: {
            main: resolve( dirname(fileURLToPath(import.meta.url)), '../src/index.html'),
            main: resolve( dirname(fileURLToPath(import.meta.url)), '../src/frame.html'),
            main: resolve( dirname(fileURLToPath(import.meta.url)), '../src/preview/index.html')
          }
        }
      },
      resolve: {
