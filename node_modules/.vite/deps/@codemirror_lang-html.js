...
function readToken(data, input, stack, group) {
  let state = 0, groupMask = 1 << group, { parser: parser4 } = stack.p, { dialect } = parser4;
  scan:
    for (; ; ) {
      ...
      for (let i = state + 3; i < accEnd; i += 2)
        if ((data[i + 1] & groupMask) > 0) {
          let term = data[i];
          if (dialect.allows(term) && (input.token.value == -1 || input.token.value == term || parser4.overrides(term, input.token.value))) {
...
var Parse = class {
  ...
  advanceStack(stack, stacks, split) {
    ...
    if (defaultReduce > 0) {
      ...
      if (verbose)
        console.log(base + this.stackID(stack) + ` (via always-reduce ${parser4.getName(defaultReduce & 65535)})`);
      return true;
    }
    ...
    for (let i = 0; i < actions.length; ) {
      ...
      if (verbose)
        console.log(base + this.stackID(localStack) + ` (via ${(action & 65536) == 0 ? "shift" : `reduce of ${parser4.getName(action & 65535)}`} for ${parser4.getName(term)} @ ${start}${localStack == stack ? "" : ", split"})`);
      if (last)
        return true;
...
function allowedChildren(doc, tree, schema) {
  let parentInfo = schema.tags[elementName2(doc, findParentElement(tree, true))];
...
var autoCloseTags2 = EditorView.inputHandler.of((view, from, to, text) => {
  ...
  let changes = state.changeByRange((range) => {
    ...
    if (text == ">" && around.name == "OpenTag") {
      if (((_b = (_a = around.parent) === null || _a === void 0 ? void 0 : _a.lastChild) === null || _b === void 0 ? void 0 : _b.name) != "CloseTag" && (name = elementName2(state.doc, around.parent, head)))
        return { range: EditorSelection.cursor(head + 1), changes: { from: head, insert: `></${name}>` } };
    } else if (text == "/" && around.name == "OpenTag") {
      let empty = around.parent, base = empty === null || empty === void 0 ? void 0 : empty.parent;
      if (empty.from == head - 1 && ((_c = base.lastChild) === null || _c === void 0 ? void 0 : _c.name) != "CloseTag" && (name = elementName2(state.doc, base, head))) {
        let insert = `/${name}>`;
        return { range: EditorSelection.cursor(head + insert.length), changes: { from: head, insert } };
      }
    }
