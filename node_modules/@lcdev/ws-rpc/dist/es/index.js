...
export function build(serializer) {
    return buildInner({}, {});
        ...
        return {
            ...
            func(name, validation) {
                ...
                if (functionValidation[name]) {
                    throw new RpcError(`Tried to call func() with validation, but a function named ${name} already exists.`, ErrorType.BuildError);
                }
            ...
            event(name, validation) {
                ...
                if (eventValidation[name]) {
                    throw new RpcError(`Tried to call event() with validation, but an event named ${name} already exists.`, ErrorType.BuildError);
                }
            ...
            client(shouldValidate = true) {
                ...
                async function connect(...args) {
                    ...
                    const [timeout, clearTimeout] = createTimeout(10000, new RpcError('Connecting to WebSocket server timed out', ErrorType.Timeout));
...
function setupClient(conn, { deserialize, serialize }, functionValidation, eventValidation, shouldValidate) {
    ...
    function assertConnectionOpen() {
        switch (conn.readyState) {
            case 1:
                return;
            case 0:
                throw new RpcError('WebSocket connection is CONNECTING, cannot perform operation', ErrorType.ConnectionNotOpen);
            case 2:
                throw new RpcError('WebSocket connection is CLOSING, cannot perform operation', ErrorType.ConnectionNotOpen);
            default:
                throw new RpcError('WebSocket connection is CLOSED, cannot perform operation', ErrorType.ConnectionNotOpen);
        }
    }
    async function messageHandler({ data: msg }) {
        ...
        if (typeof parsed !== 'object' || parsed === null) {
            logger.warn(`Received an unexpected message - deserialized as non-object. (${parsed})`);
            return;
        }
        if ('err' in parsed) {
            const { message, code, data, mid: messageID } = parsed;
            const error = Object.assign(new RpcError(message, code ? code : ErrorType.Response), { code, data });
            ...
            switch (prop) {
                ...
                default: {
                    ...
                    return async (data, timeoutMS = 15000) => {
                        ...
                        const [timeout, clearTimeout] = createTimeout(timeoutMS, new RpcError(`Call to '${prop}' failed because it timed out in ${timeoutMS}ms`, ErrorType.Timeout));
                        return Promise.race([
                            response,
            ...
            switch (prop) {
                ...
                case 'ping':
                    return async (timeoutMS = 200) => {
                        ...
                        const [timeout, clearTimeout] = createTimeout(timeoutMS, new RpcError(`Pinging all clients exceeded timeout ${timeoutMS}ms`, ErrorType.Timeout));
    ...
    function dispatch(parsed) {
        ...
        for (const handler of handlers) {
            try {
                ...
                logger.error(`An event handler for ${eventType} failed: ${normalizeError(error).toString()}`);
            }
        }
    }
...
function normalizeError(error) {
    ...
    if (error instanceof Error) {
        ...
        normalized = new RpcError(`Unknown error: ${error?.toString()}`, ErrorType.Unknown);
    }
