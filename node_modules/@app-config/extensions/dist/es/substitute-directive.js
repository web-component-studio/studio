...
export function substituteDirective(aliases = defaultAliases, environmentOverride, environmentSourceNames) {
    return named('$substitute', forKey(['$substitute', '$subs'], (value, key, parentKeys, context) => async (parse) => {
        ...
        if (value.$name) {
            logger.warn(`Detected deprecated use of $name in a $substitute directive. Use 'name' instead.`);
        }
        ...
        const parseValue = async (strValue) => {
            ...
            if (value.$parseBool) {
                logger.warn(`Detected deprecated use of $parseBool in a $substitute directive - use $parseBool directive instead`);
            }
            if (parseBool) {
            ...
            if (value.$parseInt) {
                logger.warn(`Detected deprecated use of $parseInt in a $substitute directive - use $parseInt directive instead`);
            }
            if (parseInt) {
            ...
            if (value.$parseFloat) {
                logger.warn(`Detected deprecated use of $parseFloat in a $substitute directive - use $parseFloat directive instead`);
            }
        ...
        if (value.fallback !== undefined || value.$fallback !== undefined) {
            ...
            if (value.$fallback) {
                logger.warn(`Detected deprecated use of $fallback in a $substitute directive. Use 'fallback' instead.`);
            }
            if (value.$allowNull) {
                logger.warn(`Detected deprecated use of $allowNull in a $substitute directive. Use 'allowNull' instead.`);
            }
            if (allowNull) {
